...
> **Why pfSense မှာ Suricata?** pfSense က traffic အကုန်မြင်တယ် (company-web-server, company-customer-db, DNS, LDAP ဖြတ်တဲ့ packet အကုန်)  
...
|---|---|---|
| em1 (DMZ) | ✅ **ထည့်** | company-web-server, DNS-Server ရောက်သည့် traffic အကုန် |
| em2 (INT) | ✅ **ထည့်** | company-customer-db, LDAP-Server ရောက်သည့် traffic အကုန် |
...
> WAN interface က optional ပဲ — company VMs traffic ဖမ်းဖို့ PUBLIC + INTERNAL ပဲ လုံလောက်တယ်
...
|---|---|---|
| company-web-server SSH/HTTP/FTP | 10.10.10.10 | **PUBLIC (em1.10)** |
| company-dns-server SSH/DNS | 10.10.10.20 | **PUBLIC (em1.10)** |
| company-customer-db SSH/MySQL | 10.20.20.10 | **INTERNAL (em2.20)** |
| company-ldap-server SSH/LDAP | 10.20.20.20 | **INTERNAL (em2.20)** |
...
**PUBLIC (em1.10) rules** — company-web-server + company-dns-server:

```suricata
alert tcp any any -> 10.10.10.10 22 (msg:"SSH Brute company-web-server"; flow:to_server; threshold:type both,track by_src,count 10,seconds 60; sid:9000001; rev:1;)
alert tcp any any -> 10.10.10.10 80 (msg:"HTTP Attack company-web-server"; flow:to_server,established; sid:9000002; rev:1;)
alert tcp any any -> 10.10.10.10 21 (msg:"FTP Brute company-web-server"; flow:to_server; threshold:type both,track by_src,count 10,seconds 60; sid:9000003; rev:1;)
alert udp any any -> 10.10.10.20 53 (msg:"DNS Attack company-dns-server"; sid:9000004; rev:1;)
alert tcp any any -> 10.10.10.20 22 (msg:"SSH Brute company-dns-server"; flow:to_server; threshold:type both,track by_src,count 10,seconds 60; sid:9000005; rev:1;)
```

**INTERNAL (em2.20) rules** — company-customer-db + company-ldap-server:

```suricata
alert tcp any any -> 10.20.20.10 22 (msg:"SSH Brute company-customer-db"; flow:to_server; threshold:type both,track by_src,count 10,seconds 60; sid:9000006; rev:1;)
alert tcp any any -> 10.20.20.10 3306 (msg:"MySQL Brute company-customer-db"; flow:to_server; threshold:type both,track by_src,count 10,seconds 60; sid:9000007; rev:1;)
alert tcp any any -> 10.20.20.20 22 (msg:"SSH Brute company-ldap-server"; flow:to_server; threshold:type both,track by_src,count 10,seconds 60; sid:9000008; rev:1;)
alert tcp any any -> 10.20.20.20 389 (msg:"LDAP Brute company-ldap-server"; flow:to_server; threshold:type both,track by_src,count 10,seconds 60; sid:9000009; rev:1;)
```
...
## Step 8 — SSH Key Setup (aegis-company-admin → pfSense)
...
### aegis-company-admin မှာ key ကူး

```bash
# aegis-company-admin (10.30.30.10) မှာ run
...
company VMs နဲ့ SSH tail တူတူပဲ၊ path တည်နေရာ ကွာသည်:
...
## Step 10 — company-web-server / company-customer-db မှာ Suricata ဖြုတ်

```bash
# company-web-server မှာ SSH ဝင်ပြီး
sudo systemctl stop suricata
sudo systemctl disable suricata
# (optional ဖြုတ်ချင်ရင်)
sudo apt remove --purge suricata -y

# company-customer-db မှာလည်း တူတူ
sudo systemctl stop suricata
sudo systemctl disable suricata
```
